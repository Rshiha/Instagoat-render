...
def send_audio(client, thread_id, result):
    ...
    try:
        ...
        # Also send download links (audio + video) so the user can save
        # the original files, not just play them inline.
        if base_url:
            audio_link = build_link(base_url, os.path.basename(path))
            video_link = build_link(base_url, os.path.basename(video_path))
